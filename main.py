...
# Main function behind downloading the video (AS MP4)
...
# Main function behind downloading the video (ASS Mp3 haha see what I did there)
def download_mp3(link):
    ...
    try:
        temp_file = yt_stream.download(path)
        mp3_file = Path(temp_file).with_suffix(".mp3")
        ...
        # Catch and display any errors that occur during download or conversion
        print("Download Error,", e)
...
def start_download_video():
    ...
    threading.Thread(target=download_mp4, args=(link,), daemon=True).start()
...
def start_download_audio():
    ...
    # Run the download function in a separate thread
    threading.Thread(target=download_mp3, args=(link,), daemon=True).start()
...
download_btn = tk.Button(root, text="Download MP4", command=start_download_video)
download_btn.pack(pady=5)

# Download button for em pi tri
download_btn = tk.Button(root, text="Download MP3", command=start_download_audio)
download_btn.pack(pady=5)
